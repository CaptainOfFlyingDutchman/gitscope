...
def test_identities_include_github_defaults_and_explicit_aliases(tmp_path: Path) -> None:
    ...
    assert identities.matches("unrelated", "formerOctocat@users.noreply.github.com")
    assert identities.matches("unrelated", "42+FormerOctocat@users.noreply.github.com")
    assert identities.matches("FormerOctocat", "unrelated@example.com")
...
def test_invalid_identity_line_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(IdentityFileError, match="username"):
